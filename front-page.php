<?php get_header();?>
<div class="containerFrontpage">
    <div class="middleText">
        <img class="Logo"src="<?=$img_dir?>Logo.png" alt="Logo du tim">
        <p>TECHNIQUES D'INTÉGRATIONS MULTIMÉDIA</p>
        <a href="<?$url?>/le-programme/"><p><span class="biggerC">C</span>OMMENCER LA VISITE GUIDÉE!</p><img class="flecheVisite"src="<?=$img_dir?>fleche2.png" alt="image play button"></a>
    </div>

    <div class="blocIntro">
        <img src="<?=$img_dir?>New_Piskel-1.png_1.png" alt="Timien" width="200px" height="auto">
        <div class="messageBubble">
            <p>
               BONJOUR VISITEUR, BIENVENUE SUR LE SITE DU TIM! <br>
               POUR COMMENCER UNE VISITE GUIDÉE DU DÉPARTEMENT APPUIE SUR <br> 
               LE BOUTON JOUER. TU PEUX AUSSI OUVRIR LE MENU ET Y NAVIGUER <br>
               TOI MÊME. <br><br>
               BONNE VISITE! :)
            </p>
        </div>
    </div>

    <div class="copyrightFoot">
        COPYRIGHT &copy; 2022 - TIM Maisonneuve. Tous droits réservés
    </div>
</div>
